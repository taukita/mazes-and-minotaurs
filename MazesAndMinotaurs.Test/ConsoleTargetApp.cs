...
{
	internal class ConsoleTargetApp
	{
		private const string LOOK_AT_GAME = "Посмотреть на игру";
		private const string LOOK_AT_CANVAS = "Посмотреть на 'холст'";
		private const string LOOK_AT_GRID = "Посмотреть на 'сетку'";
		private const string LOOK_AT_COMPLEX_CONTROL = "Посмотреть на составной контрол";
		...
		public void Run()
		{
			...
			var mainMenu = new MenuControl(LOOK_AT_GAME, LOOK_AT_CANVAS, LOOK_AT_GRID, LOOK_AT_COMPLEX_CONTROL, EXIT);
			...
			var complexControl = CreateComplexControl();
			complexControl.OnKeyPressed += (g, a) =>
			{
				if (a.Key == ConsoleKey.Escape)
				{
					pages.Page = 0;
			...
			pages.Controls.Add(complexControl);
		...
		private static Control CreateComplexControl()
		{
